...
function EditEnrollmentPeriodModal(props) {
  ...
  const { openModal, setOpenModal, updateEnrollmentPeriod, data } = props;
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      console.log(values);
      updateEnrollmentPeriod(data?.id, values);
    },
    validationSchema,
  });
  ...
  useEffect(() => {
    ...
    setSelectedCodeOption(
      Object.values(enrollmentPeriodCode).find((item) =>
        _.isEqual(data?.code, item.value)
      )
    );
    setSelectedTypeOption(
      Object.values(topicType)
        .map((item) => ({ label: item, value: item }))
        .find((item) => _.isEqual(data?.type, item.value))
    );
  }, [data]);
...
EditEnrollmentPeriodModal.propTypes = {
  ...
  updateEnrollmentPeriod: PropTypes.func.isRequired,
};
