...
class ApprovalHistoryController {
  public async getApprovalHistoryByTopicId(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Response
      res.status(StatusCodes.OK).json(await approvalHistoryService.getApprovalHistoryByTopicId(topicId));
    } catch (error) {
      console.log(error);
      next(error);
    }
