...
import { InternalServerErrorException, ValidateFailException } from "@exceptions";
import {
...
class DivisionService {
  public async createDivisionByTopicType(
    ...
    const isTypeValid = Object.values(TopicType).some((item) => _.isEqual(item, topicType));
    ...
    if (_.isNull(assignedlecture)) throw new ValidateFailException("Lecture could not be found");
    ...
    // Validate division
    if (_.isNull(division)) throw new InternalServerErrorException("Division could not be saved");
  ...
  public async getDivisionByTopicType(topicType: string, email: string) {
    ...
    if (_.isNull(semester)) throw new ValidateFailException("Current semester could not be found");

    // Get current lecture
    const lecture = await User.findOne({ where: { email } });
    if (_.isNull(lecture)) throw new ValidateFailException("Current lecture could not be found");
