...
import { CreateTopicEnrollmentRequest, IListTopicEnrollmentResponse, ResponseModelBuilder } from "@interfaces";
...
class TopicEnrollmentService {
  public async createTopicEnrollment(request: CreateTopicEnrollmentRequest) {
    ...
    if (!_.isEmpty(topicEnrollments)) {
      if (_.isEqual(topic.maxSlot, topicEnrollments.length) && _.isEqual(topic.availableSlot, 0)) {
        throw new ValidateFailException("Topic enrollment is full");
      }

      //           Validate user has already existed in topic enrollment
      const isAnyMatch = topicEnrollments.some((item) => _.isEqual(item.student?.ntid, request.ntid));
      if (isAnyMatch) throw new ValidateFailException("The student has already enrolled");
    }

    //        Get student
    const student = await User.findOne({ where: { ntid: request.ntid } });
    if (_.isNull(student)) throw new ValidateFailException("Student could not be found");
    ...
    if (!_.isNull(topicEnrollment))
      throw new ValidateFailException("The student has already enrolled in another topic");
  ...
  public async deleteTopicEnrollment(ntid: string) {
    //        Get user
    if (_.isNull(ntid) || _.isUndefined(ntid) || _.isEmpty(ntid)) throw new ValidateFailException("Ntid is not valid");
    const user = await User.findOne({
      where: { ntid }
    });
    if (_.isNull(user)) throw new ValidateFailException("Student could not be found");
    ...
    if (_.isNull(topicEnrollment)) throw new ValidateFailException("Topic enrollment could not be found");
  ...
  public async getTopicEnrollmentsByNtid(email: string) {
    ...
    if (_.isNull(topicEnrollment)) throw new ValidateFailException("Student has not enrolled yet");
