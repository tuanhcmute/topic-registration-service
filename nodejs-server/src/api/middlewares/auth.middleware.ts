...
export const preAuthorizeFilter = (roles: string[]) => async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      return res
        .status(StatusCodes.UNAUTHORIZED)
        .json(new ResponseModelBuilder().withStatusCode(StatusCodes.UNAUTHORIZED).withMessage(err.message).build());
    }
    ...
    // Validate user
    if (_.isNull(currentUser) || _.isEmpty(currentUser) || _.isUndefined(currentUser))
      return res
    ...
    return res
      ...
      .json(new ResponseModelBuilder().withStatusCode(StatusCodes.FORBIDDEN).withMessage("Access is denied").build());
  } catch (err) {
    next(err);
  }
};
...
// Check authorization
export const tokenAuthenticationFilter = async (req: Request, res: Response, next: NextFunction) => {
  try {
    // Permit all request
    if (req.path === "/api/v1/oauth2/authorization/google" || req.path === "/api/v1/login/oauth2/code/google")
      return next();
