...
public class TopicServiceImpl implements TopicService {
        ...
        @LoggerAround
        @Override
        public Response<List<TopicDTO>> getAllTopicsInLectureEnrollmentPeriod(String type, Integer pageNumber,
                ...
                // Validate type
                boolean isMatch = Arrays.stream(TopicType.values())
                ...
                if (!userOptional.isPresent())
                        throw new BadRequestException("Lecture could not be found");
                User lecture = userOptional.get();

                // Get topics
                Page<Topic> topicPage = topicRepository.findBySemesterAndTypeAndLecture(currentSemester, topicType,
                                lecture, paging);
        ...
        public Response<Void> createNewTopicInLectureEnrollmentPeriod(NewTopicRequest request) {
                ...
                if (!majorOptional.isPresent())
                        throw new BadRequestException("Major could not be found");
                ...
                if (semesters.isEmpty())
                        throw new BadRequestException("Current semester could not be found");
                Semester currentSemester = semesters.get(0);
                ...
                if (!students.isEmpty()) {
                        List<TopicEnrollment> topicEnrollments = students.stream()
                                        .map(student -> TopicEnrollment.builder()
                                                        .student(student)
                                                        .topic(topic)
                                                        .build())
                                        .collect(Collectors.toList());
                        topicEnrollmentRepository.saveAll(topicEnrollments);
                        topicEnrollments.forEach(
                                        topicEnrollment -> log.info("Topic enrollment: {}", topicEnrollment.getId()));
                }
